...
bool License::load(const std::string &licenseBase64) {
  try {
    ...
    throw LicenseException("Failed to load the license: " +
                           std::string(e.what()));
  }
  return false;
}
...
std::string License::generate_kv_pair(std::uint32_t length,
                                      const std::string &secret) {
  Ripe::KeyPair pair = Ripe::generateRSAKeyPair(length, secret);
  if (pair.privateKey.empty() || pair.publicKey.empty()) {
    throw std::runtime_error("Failed to generate key pair.");
  }
  return Ripe::base64Encode(pair.privateKey) + ":" +
         Ripe::base64Encode(pair.publicKey);
}
